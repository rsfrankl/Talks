...
ca_county_data <- read_excel("~/Documents/GitHub/SCAG/California_age_2022_2023.xlsx", sheet = "CA_estimates")
...
ggplot(ca_county_data, aes(County, pop_change_22_23, colour = SCAG)) +
  geom_point()+
  coord_flip()
...
new_data <- ca_county_data %>%
  mutate(SCAG = as.factor(SCAG))
ggplot(new_data) +
  geom_segment( aes(x=County, xend=County, y=0, yend=pop_change_22_23), color="grey") +
  geom_point( aes(x=County, y=pop_change_22_23, color=SCAG, size=total_2024)) +
  coord_flip() +
  ...
  xlab("") +
  ylab("California County Population Change (%), Jan 2023-Jan 2024 (CA Dept of Finance)")
